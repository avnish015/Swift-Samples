...
class AppController: NSObject,NSMenuDelegate,NSUserNotificationCenterDelegate
{
    ...
    deinit{
       ...
       NotificationCenter.default.removeObserver(self, name: NSNotification.Name(rawValue: DSBatteryCapacityThresholdDidChangeNotification), object: nil)
    }
    ...
    func configureBatteryStatus()
    {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(AppController.batteryCapacityThresholdDidChange(_:)), name: NSNotification.Name(rawValue: DSBatteryCapacityThresholdDidChangeNotification), object: nil)
...
// MARK: - KVO
    ...
    {
        if ((object! as AnyObject).isEqual(self.sleepWakeTimer) && keyPath == "scheduled")
        {
