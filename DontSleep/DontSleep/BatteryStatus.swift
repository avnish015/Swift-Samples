...
//Returns the current battery charging level of the internal battery, or BatteryStatusUnavailable.
    ...
    */
    func registerForCapacityChangesIfNeeded()
    {
        ...
        let  runLoopSource = IOPSNotificationCreateRunLoopSource(DSBatteryStatusChangeHandler as! IOPowerSourceCallbackType, UnsafeMutablePointer(Unmanaged.passUnretained(self).toOpaque()))
...
func DSBatteryStatusChangeHandler(_ context:UnsafeMutableRawPointer)
{
    ...
    let batteryStatus = Unmanaged<BatteryStatus>.fromOpaque(OpaquePointer(context)).takeUnretainedValue()
